...
if task == 'caltech':
	dataset = []
	dataset.append(np.array(glob.glob('/face/*png')))
	dataset.append(np.array(glob.glob('/motor/*png')))
...
if needs_training:
	...
	for i in range(iteration):
		if image_idx == len(training_set):
			...
			# stop if there are silent images
			if perf[2] > 0:
				break
			...
			if best_test[0] <= perf[0]:
				best_test = perf
				torch.save(weights, "MozafariShallow_S2_"+task+".pt")
	...
	# S1 Features #
	weights = torch.load("MozafariShallow_S2_"+task+".pt")
	feature,cstride = st.get_deep_feature(feature, cstride, (receptive_field,receptive_field), (1,1), weights)
	for i in range(number_of_features):
		st.plot_tensor_in_image('feature_s2_'+str(i).zfill(4)+'.png',feature[i])

	f = open('Perf_MozafariShallow.txt', 'w')
